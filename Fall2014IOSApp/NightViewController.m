...
- (NSString *)getYelpCategoryFromSearchText {
    ...
    if ([[self.searchTextField.text componentsSeparatedByString:@" nightspot"] count] > 1) {
        NSCharacterSet *separator = [NSCharacterSet whitespaceAndNewlineCharacterSet];
        NSArray *trimmedWordArray = [[[self.searchTextField.text componentsSeparatedByString:@"nightspot"] firstObject] componentsSeparatedByCharactersInSet:separator];
    ...
    else if (([[self.searchTextField.text componentsSeparatedByString:@" nightspot"] count] <= 1)
...
- (void)findNearByRestaurantsFromYelpbyCategory:(NSString *)categoryFilter {
    if (categoryFilter && categoryFilter.length > 0) {
        if (([CLLocationManager authorizationStatus] != kCLAuthorizationStatusDenied)
            ...
            self.yelpService = [[YelpAPIServiceNight alloc] init];
...
-(void)loadResultWithDataArray:(NSArray *)resultArray {
    ...
    if ([self.tableViewDisplayDataArray count] == 1) {
        // 2
        [self.vocalizer speakString:[NSString stringWithFormat:@"I found %lu %@ nightspot",
                                     (unsigned long)[self.tableViewDisplayDataArray count],
                                     self.searchCriteria]];
    }
        ...
        // 2
        [self.vocalizer speakString:[NSString stringWithFormat:@"I found %lu %@ nightspots",
                                     (unsigned long)[self.tableViewDisplayDataArray count],
                                     self.searchCriteria]];
    }
    
    else {
        [self.vocalizer speakString:[NSString stringWithFormat:@"I could not find any %@ nightspots",
                                     self.searchCriteria]];
    }
}
